...
func TestNewComponentLoggerEmitsJSONWithComponent(t *testing.T) {
	...
	logger := newComponentLogger("connection-test-executor", &output, "DEBUG")
	...
	if record["component"] != "connection-test-executor" {
		t.Fatalf("component = %v, want connection-test-executor", record["component"])
	}
