...
// metricsServer hosts the /metrics endpoint on a dedicated port for the
// Connection Test Executor binary. The deployment defaults to :9090;
// the Prometheus scrape configuration in deployment/helm/astrasync
// matches.
func metricsServer(ctx context.Context, logger *slog.Logger, listenAddress string) (*http.Server, error) {
	...
	server := &http.Server{
		Handler:           connectiontestmetrics.Handler(),
